...
void uart_init()
{
  ...
  //	     (1 << 7) | (1 << 8) | (1 << 9) | (1 << 10));
  // mmio_write(UART0_IMSC, 0x10); // Receive interrput mask
...
void kernel_main(uint32_t r0, uint32_t r1, uint32_t atags)
{
  ...
  interrupt_init();
  uart_puts("INTERRUPTS OK !\r\n");
  uart_puts("Initializing MMU ...\r\n");
  enable_mmu(MMUTABLEBASE, ~0); /* enables the MMU with TLB at MMUTABLEBASE and give acces to all supervisors */
  uart_puts("MMU OK !\r\n");
